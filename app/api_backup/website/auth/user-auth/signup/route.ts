...
const EMAIL_COOKIE_MAX_AGE = 5 * 60; // 5 minutes (in seconds)
...
export async function POST(req: Request) {
    try {
        ...
        if (existing) {
            ...
            res.cookies.set("encryptedEmail", emailPayload, {
                httpOnly: true,
        ...
        try {
            ...
            if (createdUser?.id) {
                ...
                console.log("Updated meta:", updatedUser);
            }
        } catch (e) {
        ...
        res.cookies.set("encryptedEmail", encrypted, {
            httpOnly: true,
            secure: process.env.NODE_ENV === "production",
            sameSite: "lax",
            maxAge: EMAIL_COOKIE_MAX_AGE,
