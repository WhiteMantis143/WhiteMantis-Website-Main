...
export async function POST(req: Request) {
  try {
    const { otp, encryptedEmail } = await req.json();
    ...
    // Check if user is new: temp_user exists and equals "true"
    // For existing users, temp_user won't exist (undefined), so this will be false
    ...
    // isNewUser will be true only if temp_user metadata exists and equals "true"
    let isNewUser = isTempUser;
