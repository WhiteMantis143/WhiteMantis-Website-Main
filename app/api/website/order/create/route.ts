...
export async function POST(req: NextRequest) {
  try {
    ...
    if (address?.saveAddress && address.shipping && session?.user) {
      try {
        ...
        const response = await fetch(
          `${process.env.NEXTAUTH_URL}/api/website/profile/address/update`,
          {
            method: "POST",
            headers: {
    ...
    if (session?.user) {
      if (deliveryOption === "ship") {
        ...
        payload = {
          ...
          billing: {
            first_name: address.shippingAsbillingAddress
              ? address.shipping.firstName
              : address.billing.firstName,
            last_name: address.shippingAsbillingAddress
              ? address.shipping.lastName
              : address.billing.lastName,
            address_1: address.shippingAsbillingAddress
              ? address.shipping.address
              : address.billing.address,
            city: address.shippingAsbillingAddress
              ? address.shipping.city
              : address.billing.city,
            state: address.shippingAsbillingAddress
              ? address.shipping.state
              : address.billing.state,
            postcode: address.shippingAsbillingAddress
            ...
            ? [
                {
                  rate_id: taxRateId,
                  tax_total: String(
                    ((finalPrice + shippingCost) * taxPercent) / 100,
                  ),
                  label: taxLabel,
                },
              ]
    ...
    try {
      ...
      console.log(
        "📦 Sending to WooCommerce - coupon_lines:",
