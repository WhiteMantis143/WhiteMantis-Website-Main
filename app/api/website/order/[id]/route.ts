...
export async function GET(
    ...
    if (session?.user) {
        ...
        // Guest user - retrieve using guest access token
        console.log("🔍 Guest user accessing order:", id);

        // Get token from query parameter
        const token = req.nextUrl.searchParams.get('token');

        if (!token) {
            console.log("❌ No guest access token provided");
        ...
        try {
            ...
            const storedToken = order.meta_data?.find((meta: any) => meta.key === "guest_access_token")?.value;

            if (!storedToken || storedToken !== token) {
                console.log("❌ Invalid guest access token");
                return NextResponse.json({ success: false, message: "Unauthorized - invalid token" }, { status: 403 });
            }

            console.log("✅ Guest token validated successfully");

            // Fetch Stripe Payment Details (optional for guests)
            let paymentDetails = null;
            let paymentIntents = [];
            ...
            try {
                ...
                if (paymentSearch.data.length > 0) {
                    paymentIntents = paymentSearch.data;
                    paymentDetails = paymentIntents[0];
                }
            } catch (stripeError) {
                console.error("Error fetching Stripe payment details:", stripeError);
            }
