...
const GridFilter = () => {
  ...
  const DEFAULT_PARENT_SLUG = "coffee-drip-bags";

  // mounted ref and per-page constant
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        let parentId = null;
        if (Array.isArray(slugJson) && slugJson.length) parentId = slugJson[0].id;
        // If the parent category doesn't exist, mark resolved but missing so we DON'T
        // fall back to listing all products (that was causing coffee-beans products to show).
        if (!parentId) {
          if (!mountedRef.current) return;
          setFilters([]);
          setProducts([]);
        ...
        // Parallelize child category fetches instead of sequential loop
        const builtFilters = await Promise.all(
          groups.map(async (g) => {
            ...
            return { key: String(g.slug || g.id), label: g.name, options };
          })
        );

        if (!mountedRef.current) return;
        setFilters(builtFilters.length ? builtFilters : [{ key: 'category', label: 'Category', options: groups.map(g => ({ id: g.id, name: g.name })) }]);
        setPage(1);
        await loadProducts(parentId, [], 1, false);
      } catch (err) {
        console.error("Failed to load categories/products:", err);
        setPage(1);
        await loadProducts(null, [], 1, false);
      }
    }
  ...
  useEffect(() => {
    ...
    async function reFetch() {
      ...
      try {
        // Parallelize filter fetches instead of sequential loop
        const groupResults = await Promise.all(
          selectedGroups.map(async (key) => {
      ...
      <div className={styles.Main}>
        <div className={styles.MainConatiner}>
          <div className={styles.TopConatiner}>
            ...
            <div className={styles.SortFilter}><p>Sort bY:</p><p>Recommended</p></div>
          </div>

          <div className={styles.BottomConatiner}>
            <div className={styles.LeftFilters}>
              <div className={styles.FilterHeading}><p>Filter By :</p></div>
              <div className={styles.Filters}>
                {(filters.length ? filters : []).map((f) => {
                  const hasSelected = (selected[f.key] || []).length > 0;
                  const isOpen = (openPanels || []).includes(f.key);
                  return (
                    <div key={f.key} className={styles.FilterBlock}>
                      <div className={styles.FilterHeader} onClick={() => togglePanel(f.key)} aria-expanded={isOpen}>
                        <h4>{f.label}</h4>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          {hasSelected ? (
                            <button className={styles.ClearBtn} onClick={(e) => { e.stopPropagation(); clearFilter(f.key); }}>×</button>
                          ) : null}
                      ...
                      {isOpen ? (
                        <div className={styles.CheckboxList}>
                          {f.options && f.options.length ? f.options.map((opt) => (
                            <label key={opt.id} className={styles.CheckboxItem}>
                              <input type="checkbox" checked={(selected[f.key] || []).includes(opt.id)} onChange={() => toggleOption(f.key, opt.id)} />
                              <span>{opt.name}</span>
                            </label>
                          )) : (
                            <div className={styles.CheckboxListEmpty} style={{ color: '#fff' }}>No options</div>
                          )}
                        </div>
                      ) : null}
                    </div>
            ...
            <div className={styles.RightProducts}>
              <div className={styles.Products}>
                {products.length ? products.map((product) => {
                  ...
                  const imgSrc = (product.images && product.images[0] && product.images[0].src) || null;
                  return (
                    <div key={product.id || product.slug || Math.random()} className={styles.Product}>
                      <Link href={`/products/${product.id}`}>
                        <div className={styles.ProductsTop}>
                          <div className={styles.ProductTopOne}>
                            <div className={styles.WishList}><WishlistButton product={product} /></div>
                            <div className={styles.ProductImgg}>
                              {imgSrc ? (
                                <Image src={imgSrc} alt={title} width={400} height={350} />
                              ) : (
                                <div style={{ width: 400, height: 350, background: '#f4f4f4' }} />
                              )}
                            </div>
                          </div>
                          <div className={styles.ProductTopTwo}>
                            <h2>{title}</h2>
                            <p>{truncateText(stripHtml(desc), 100)}</p>
                          </div>
                        </div>
                      </Link>
                      <div className={styles.ProductsBottom}>
                        <p>{formatFinalPrice(product)}</p>
                        <button className={styles.AddToCart} onClick={() => addItem(product.id, 1)}>Add to cart</button>
                      </div>
                    </div>
                  );
                }) : (
                  <div style={{ padding: 24 }}>No items available</div>
                )}
              </div>
